(() => {
    ...
    Polymer({
        ...
        _zoomScaleChange: function(newValue, location) {
            ...
            let stepX = nowLocation.x - preLocation.x,
                stepY = nowLocation.y - preLocation.y
        ...
        _doSaveFunc: function() {
            if (this._openPath == null) {
                let newPath = Electron.remote.dialog.showSaveDialog({
                    title: '请选择您要保存的路径',
                    defaultPath: window.projectFolder,
                    filters: [
        ...
        mouseupTouchnull: function(e) {
            let canvas = this.$.scene.getFabricCanvas()
            if (isCtrlKey(e) || canvas.getActiveObject() || canvas.getActiveGroup()) {} else {
                canvas.clear()
                this._firstSelectItem = null
            }

            this.preSelectorRect({
                selector: {
                    ex: e.offsetX,
                    ey: e.offsetY,
                    left: 0,
